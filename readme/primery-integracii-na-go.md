...
Код для создания нового платежного заказа через API Panda Pay на языке Go.
...
* `1000.47474483483843894` → сумма заказа в рублях
* `"SBP"` → тип платежной системы

***

### Сервер для обработки webhook (Go)

Код HTTP-сервера для приема и проверки callback-уведомлений на языке Go.
...
* `apiKey` → ваш API ключ
* `secret` → ваш секретный ключ
* `"1000.47474483483843894"` → сумма заказа (строковый формат)
* `"SBP"` → тип платежной системы
